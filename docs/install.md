...
> OpenCode is different: it uses a TypeScript plugin generated by
> `install-hooks --agent opencode`, so no shell-script extraction is
> needed for that client.
...
The curl script installer supports `--agent codex|claude-code` and
`--to <dir>`; `--help` prints the full flag list. OpenCode does not
need this script installer because `install-hooks --agent opencode`
generates a TypeScript plugin instead.
